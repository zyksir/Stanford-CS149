...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  //
  __cs149_vec_float x, result, upperBound = _cs149_vset_float(9.999999f);
  __cs149_vec_int y, zero = _cs149_vset_int(0), one = _cs149_vset_int(1);
  __cs149_mask maskAll, maskIfYNotZero, maskExceedUpperBound;
  maskAll = _cs149_init_ones();
  for(int i=0; i<N; i+=VECTOR_WIDTH) {
    _cs149_vload_float(x, values+i, maskAll);       // float x = values[i];
    _cs149_vload_int(y, exponents+i, maskAll);      // int y = exponents[i];
    _cs149_vgt_int(maskIfYNotZero, y, zero, maskAll);  // if (y == 0) {
    result = _cs149_vset_float(1.0f);               // output[i] = 1.f;
    while(_cs149_cntbits(maskIfYNotZero) != 0) {    // while (count > 0) {
      _cs149_vmult_float(result, result, x, maskIfYNotZero); // result *= x;
      _cs149_vsub_int(y, y, one, maskIfYNotZero); // y--;
      _cs149_vgt_int(maskIfYNotZero, y, zero, maskAll);  // if (y == 0) {
    }
    _cs149_vgt_float(maskExceedUpperBound, result, upperBound, maskAll); // if (result > 9.999999f) 
    _cs149_vmove_float(result, upperBound, maskExceedUpperBound);        // result = 9.999999f;
