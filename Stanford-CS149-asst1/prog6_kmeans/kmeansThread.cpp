...
void computeAssignmentThread(WorkerArgs *const args) {
  static int NUM_THREADS = 8;
  double *minDist = new double[args->M];

  // Initialize arrays
  memset(args->clusterAssignments, -1, sizeof(args->clusterAssignments[0])*(args->M));
  memset(reinterpret_cast<int*>(minDist), -1, sizeof(minDist[0])*(args->M));

  MyArgs myArgs[NUM_THREADS];
  std::thread workers[NUM_THREADS];
  int numPerThread = args->M / NUM_THREADS;
  auto f = [args, minDist](MyArgs *const myArg) {
    for (int k = args->start; k < args->end; k++) {
      for (int m = myArg->startM; m < myArg->endM; m++) {
        double d = dist(&args->data[m * args->N],
                        &args->clusterCentroids[k * args->N], args->N);
        if (*reinterpret_cast<int*>(minDist + m) == -1 || d < minDist[m]) {
          minDist[m] = d;
  ...
  for (int i=0; i<NUM_THREADS; i++) {
    myArgs[i].startM = i*numPerThread;
    myArgs[i].endM = (i+1)*numPerThread;
    workers[i] = std::thread(f, &myArgs[i]);
  }
...
void kMeansThread(double *data, double *clusterCentroids, int *clusterAssignments,
  ...
  double timeComputeAssignments = 0, timeComputeCentroids = 0, timeComputeCost = 0;
  double startTimeLoop = CycleTimer::currentSeconds();
  while (!stoppingConditionMet(prevCost, currCost, epsilon, K)) {
    ...
    computeAssignmentThread(&args);
	  // computeAssignments(&args);
    endTime = CycleTimer::currentSeconds();
    timeComputeAssignments += (endTime - startTime) * 1000;

    startTime = CycleTimer::currentSeconds();
    computeCentroids(&args);
    endTime = CycleTimer::currentSeconds();
    timeComputeCentroids += (endTime - startTime) * 1000;

    startTime = CycleTimer::currentSeconds();
    computeCost(&args);
    endTime = CycleTimer::currentSeconds();
    timeComputeCost += (endTime - startTime) * 1000;

    iter++;
  }
  double endTimeLoop = CycleTimer::currentSeconds();
  printf("timeCost in computeAssignments: %f ms\n", timeComputeAssignments);
  printf("timeCost in computeCentroids: %f ms\n", timeComputeCentroids);
  printf("timeCost in computeCost: %f ms\n", timeComputeCost);
  printf("timeCost in Loop: %f ms with %d loop\n", (endTimeLoop - startTimeLoop) * 1000, iter);
